...
const loadProducts = () => {
  ...
  // show products function calling 
  showProducts(data);
};
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `
      <div class="single-product">
            ...
            <h4>Review: ${product.rating.rate}</h4>
            <h4>Total Ratings: ${product.rating.count}</h4>
            <button onclick="addToCart(${product.id},${product.price})"   id="addToCart-btn" class="buy-now btn btn-primary">add to cart</button>
            <button id="details-btn" class="btn btn-secondary">Details
            </button>  
      </div>
      `;
    // for displaying details on the ui we need to uncomment this
    /*
     <button id="details-btn" class="btn btn-ebuy-secondary" onclick="getId(${product.id})">Details</button> 
     */
...
const addToCart = (id, price) => {
  count = count + 1;
  // update Price Function calling 
  updatePrice("price", price);
  // update tax and charge calling 
  updateTaxAndCharge();
...
const updatePrice = (id, value) => {
  // get input Value function calling 
...
const updateTotal = () => {
  const grandTotal =
    // get input value of price, delivery charge and total tax calling
    getInputValue("price") + getInputValue("delivery-charge") +
    getInputValue("total-tax");
  document.getElementById("total").innerText = parseFloat(grandTotal.toFixed(2));
};
// load products function calling 
loadProducts();
...
/* const getId = id => {
  const idNumber = id;
  fetch(`https://fakestoreapi.com/products/${idNumber}`)
    .then(res => res.json())
    .then(data => disPlaySingleItemsDetail(data));
} */

/* const disPlaySingleItemsDetail = data => {
  const divContainer = document.getElementById('single-item');
  const div = document.createElement('div');
  divContainer.innerHTML = '';
  // div.classList.add('w-50', 'mx-auto', 'single-product');
  div.innerHTML = `
   <div class="card">
